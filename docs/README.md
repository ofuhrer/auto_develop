...
- phase-aware context bundles with manifests that record included and omitted inputs, bundle size, and truncation/omission records;
...
- bundle-manifest evidence such as `worker_context_manifest.json` for context shaping only, not semantic retrieval, embeddings, or broad cross-run search;
...
- semantic retrieval/search, embeddings, and broader cross-run retrieval remain future work even though context bundles are already phase-aware and manifest-backed;
