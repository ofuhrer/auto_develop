...
- duplicate findings are classified as deferred non-blocking findings (`deferred_finding_ids`, not `accepted_finding_ids`) with evidence-backed rationale captured in typed supervisor decision artifacts, while false-positive findings are the non-blocking acceptance path and remain auditable in `feature_review.json` / `feature_review_recheck.json`;
