...
The intended workflow is autonomous-first. Humans define the repository goal and hard policy boundaries; the current governor service chooses one epic at a time, produces an objective, and feeds the existing objective/contract/release machinery. The runtime supervisor records structured repair/resume evidence for recoverable release failures. The codebase also supports deterministic state-review snapshot artifacts (`state_review_snapshot.json`) and contract-plan references (`state_review_snapshot_path`) as bounded evidence inputs. The task/review/repair pipeline now also emits phase-aware context bundle manifests such as `worker_context_manifest.json`; those manifests record the requested phase, included and omitted sections, size metrics, and deterministic truncation or omission decisions so operators can see why a bundle was slimmed. When `model_roles.reviewer` is configured, `run-release` performs an independent feature-review pass after accepted tasks are integrated into `feature/<release>`, persists `feature_review.json` and `feature_review_recheck.json`, launches bounded repair contracts for required findings, reruns verification, and blocks finalization while unresolved required findings remain. When the bounded review budget is exhausted or convergence stops, the system reruns final integration verification on the integrated feature branch before the supervisor performs final adjudication of the remaining findings. Final review stop hardening uses that rerun as the convergence boundary: blocker findings stay blocked, soft and false-positive or verification-only findings may be accepted only with rationale after the rerun passes, duplicate findings are deferred to next-cycle follow-up, and scope-expansion or backlog-follow-up findings are recorded as repo-state follow-up proposals instead of being folded into the current release. That review pass now leaves a first-class artifact trail for the review prompt, raw reviewer output, normalization decision, review-repair artifacts, recheck, final verification evidence, final review-continuation decision, generated repair contracts, supervisor adjudication records, and backlog follow-up proposals. `release_review.md` stays the deterministic release summary, while the reviewer JSON artifacts capture the semantic reviewer pass and the repair-contract trail. Release-local convergence is evidence-driven: accepted or deferred outcomes are compacted into the configured repo-state memory through `StateStore` so the next planning cycle can read a short durable summary without replaying the raw review logs. Planner-output normalization is implemented for repairable schema drift: the supervisor can backfill derivable evidence paths, normalize worktree-local verification commands to the configured shared runtime, and persist typed decision evidence with validator rerun requirements, but it must refuse if the repair would broaden scope, change intent, or invent missing hard evidence. Reviewer- and supervisor-output normalization extensions remain planned until code and tests explicitly ship them. This review loop is intentionally release-local: it is the implemented semantic gate for one integrated feature branch, not the broader multi-epic governor. The broader multi-epic governor loop, semantic retrieval/search, embeddings, and fully agent-driven pre-epic state-review decisioning remain planned beyond the current one-epic governor boundary.

Operational artifacts to check:

- `runs/<run-id>/release.log` is the release cockpit.
- `runs/<run-id>/events.jsonl` carries structured release events, and repeated-cycle runs also write parent `governor.log`.
- `runs/<run-id>/release_review.md` is the deterministic summary.
- `runs/<run-id>/feature_review.json`, `runs/<run-id>/feature_review_recheck.json`, and `runs/<run-id>/supervisor_decisions/final_review_finding_adjudication__<id>.json` are the semantic review and final adjudication trail.
- Repo-state follow-up summaries are compacted under the release-scoped epic record so the next planner cycle can consume them without re-reading raw reviewer logs.
...
Overlap handling is now supervisor-owned for normal source files. `run-release` writes an overlap-risk report plus a typed `release_scheduling` decision artifact, and the selected action can serialize overlapping work or keep independent tasks in parallel. The broader autonomous workflow is still not "no overlap at all." The governor should minimize overlap, classify overlap risk, and decide whether tasks can run in parallel, must be chained, should use stacked branches, or need an explicit merge-repair plan. Deterministic code should only hard-block unsafe overlap such as generated artifacts, lockfiles, migrations, configured exclusive paths, forbidden paths, or files outside task scope.

The implemented soft scope-risk policy follows the same pattern for changed-file and diff-size overages. Those overages are supervisor-adjudicated findings only after the hard gates pass. During `run-release`, if an overage is present and no scope-risk decision artifact exists yet, the release writes a deterministic typed `scope_risk_budget_policy` decision artifact in the same run under `runs/<run-id>/supervisor_decisions/` and blocks finalization until an explicit acceptance decision (`accepted_with_guards`) is present. Hard gates still stop the release for forbidden paths, generated artifacts, lockfiles, migrations, configured exclusive paths, out-of-scope files, missing evidence, unsafe finalization, and failed verification.

For `auto_develop` self-development, `src/agentic_devloop/release.py` is intentionally not listed as an unsafe overlap path in `configs/auto_develop.yaml`. It is a frequent integration point for release orchestration epics, so exact overlap should produce an overlap-risk report and supervisor scheduling decision rather than a hard pre-execution stop. The remaining guardrails are explicit `depends_on`, serialized execution for risky overlap, contract `allowed_files`, forbidden changes, verification, feature review, and merge isolation on `feature/<release>`.

After the run, inspect `release_metrics.json`, `release_budget.json`, `release_tuning.md`, and `supervisor_decisions/cost_runtime_governance__<release-id>.json` alongside the release log. The budget ledger records usage, task-size outliers, verification bottlenecks, and waste signals; the tuning report translates those signals into guidance for the next run. The cost-runtime decision artifact records `selected_action`, `outcome`, `selected_model_role`, `budget_class`, `risk_level`, `fallback_plan`, `validators_to_rerun`, and the release evidence paths that justified the routing choice. Its runtime behavior is conservative: if metrics are absent or unreadable, the governor falls back to decomposed execution rather than guessing, and hard gates still override any routing preference.
...
Soft-gate decisions are written next to the evidence they describe. Task-level exceptions land in `runs/<run-id>/<task-id>/evidence/soft_gate_decision.json` and also emit a typed supervisor decision record under `runs/<run-id>/<task-id>/evidence/supervisor_decisions/soft_budget_acceptance__<id>.json`; release-level budget exceptions land in `runs/<run-id>/soft_gate_decisions.json`. Each soft-gate record stores `finding_id`, `severity`, `risk`, `recommended_actions`, `evidence_paths`, `decision`, `rationale`, `fallback_plan`, and `validators_rerun`; typed supervisor decision records store the same rationale and evidence references plus normalized fields such as `budget_name`, `configured_limit`, `actual`, and `outcome`. Release scheduling decisions are stored under `runs/<run-id>/supervisor_decisions/release_scheduling__<release-id>__scheduling.json` and include `selected_action`, `fallback_plan`, `validators_to_rerun`, and strict staleness inputs so a stale artifact fails deterministically instead of falling back silently. These typed records are the implemented audit trail for supervisor-owned scheduling, repair continuation, finding adjudication, contract normalization, and environment repair, but they do not bypass hard validation. The rerun list is the audit trail for which checks the reviewer or supervisor should repeat or re-read before treating the soft exception as durable, and normalization is only acceptable when the candidate artifact stays inside the original objective and allowed scope.
...
If a release needs repair or resume, also inspect `runs/<release-run-id>/runtime_supervisor/` for structured release events, retry budgets, release-summary references, and repair evidence. Verification-environment repair is the narrow case to look for in task bundles: it writes `verification_environment_repair_input.json` and a typed `environment_repair__<id>.json` decision under `runs/<run-id>/<task-id>/evidence/supervisor_decisions/`. When the decision is `apply_repair_and_retry`, the retry consumes one retry-budget slot and reruns the task verification path; `capture_evidence_only` records the environment evidence without retrying, and `stop` or `escalate` preserve the refusal trail. This seam is only for verification-environment drift and bounded evidence capture, not executor-liveness supervision, dependency installation, network repair, credential repair, or source-file mutation.
...
- The parent cockpit records planning, state review, backlog selection, contract generation, child release start/completion, feature review, repair decisions, final verification, finalization decision/completion, cleanup eligibility, stop reasons, and next-epic selection.
- Cockpit event compatibility is explicit: `events.jsonl` keeps legacy top-level keys (`timestamp`, `event_type`, `message`, `artifacts`) and adds typed `context`. New consumers should prefer typed `context.phase`, `context.cycle_index`, `context.epic_id`, `context.release_id`, `context.decision`, and `context.outcome`; legacy/generic events remain compatibility events until a deprecation plan exists.
- `events.jsonl` uses 1-based `context.cycle_index`. The `next_epic_selected` event is emitted after completing cycle `N`, but its `context.cycle_index` is `N+1` and its `context.epic_id` / `context.release_id` refer to the upcoming cycle's selection (not the release that just completed).
- Parent stop metadata uses the fixed `stop_reason` taxonomy and the derived `stop_context.category` values `no_actionable_work`, `planning_only_strategy`, `non_accepted_release`, `blocked_finalization`, `state_refresh_failure`, `missing_planner_credentials`, `hard_policy_stop`, `repeated_epic_selected`, `requested_epic_count_reached`, or `unknown`. Exception-path categorization is intentionally best-effort and currently message-based for missing credentials and hard-policy failures; the `stop_reason_recorded` event includes the exception class in `context.details.exception_class` to make ambiguous cases inspectable.
...
- Child release evidence stays in the child run. `release.log`, `release.raw.log`, `release_review.md`, `release_metrics.json`, `release_budget.json`, `release_tuning.md`, `feature_review.json`, `feature_review_recheck.json`, and `supervisor_decisions/final_review_finding_adjudication__<id>.json` remain linked and inspectable without being rewritten by the parent cockpit. `BacklogEvidenceManifest` guarantees links when the child release produced them; paths such as feature-review output, final verification, finalization, cleanup, and repo-state proposal artifacts are opportunistic and omitted when the child release did not reach that phase.
...
When `blocked_by_hard_gate` occurs during final adjudication, the release writes a `FinalReviewContinuationDecision` with `hard_stop_reason` set to either `missing_release_evidence` (missing required release artifacts like `feature_review.json`, final integration verification evidence, or verification logs) or `schema_invalid_reviewer_output` (reviewer output could not be normalized/validated). The corresponding raw log events are `event=final_review_missing_release_evidence` and `event=final_review_schema_invalid_reviewer_output`. When final adjudication writes per-finding supervisor decisions, the continuation decision also includes `finding_adjudication_paths` so the release summary can link each `supervisor_decisions/final_review_finding_adjudication__*.json` artifact.
...
The implemented adjudication path is intentionally narrow: after the final integration verification rerun, it can accept false-positive or verification-only findings with rationale, defer duplicate/scope-expansion/backlog-follow-up findings to the next cycle, and it does not accept findings that require implementation changes. Final adjudication evidence comes from release artifacts (`feature_review.json`, final integration verification evidence, and verification logs when present); reviewer finding `evidence_paths` are optional context and are not required for accepted/deferred final classifications. That keeps the reviewer loop bounded to the integrated feature branch while leaving broader multi-epic review orchestration and full live pre-epic state-review decisioning as planned work. Typed supervisor decision records already exist for the bounded soft-budget, repair, normalization, and release-local finding-adjudication decisions that support this loop.
Compacted repo-state follow-up memory always stores non-empty `evidence_paths`; when reviewer-provided evidence references are missing, compaction backfills those paths from the release-scoped artifact trail so the durable summary remains auditable. These compact follow-up memories are stored on a repo-state epic record keyed by `release_id` (the release-scoped identity), not under the backlog epic that originally scheduled the release.
...
- duplicate findings are classified as deferred non-blocking findings (`deferred_finding_ids`, not `accepted_finding_ids`) with evidence-backed rationale captured in typed supervisor decision artifacts, while false-positive and verification-only findings are the non-blocking acceptance path and remain auditable in `feature_review.json` / `feature_review_recheck.json`;
- soft findings remain auditable decisions rather than silent ignores;
- scope-expansion findings become proposal material for the next planning cycle instead of broadening the current release;
- accepted and deferred findings are compacted into repo-state memory so the next planning cycle can see the final outcome without rereading the raw review logs;
...
- Typed supervisor decision records are implemented for bounded scheduling, repair, and soft-budget decisions; normal source overlap now routes through a scheduling decision artifact, while the N-epic loop that would consume them across repeated epics remains planned.
