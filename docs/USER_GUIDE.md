...
The implemented soft scope-risk policy follows the same pattern for changed-file and diff-size overages. Those overages are supervisor-adjudicated findings only after the hard gates pass. Hard gates still stop the release for forbidden paths, generated artifacts, lockfiles, migrations, configured exclusive paths, out-of-scope files, missing evidence, unsafe finalization, and failed verification.
