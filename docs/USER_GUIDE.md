...
Compacted repo-state follow-up memory always stores non-empty `evidence_paths`; when reviewer-provided evidence references are missing, compaction backfills those paths from the release-scoped artifact trail so the durable summary remains auditable.
