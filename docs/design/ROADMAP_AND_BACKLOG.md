...
1. `planner-admission-repair` is now the first operational gap because the planner can still emit semantically useful but admission-invalid contract packages. A high-level supervisor should repair/split/narrow/replan instead of requiring manual materialization.
2. `governor-cockpit-v2` follows because multiple child logs are already operationally expensive. The target operator experience is one colorful, human-facing cockpit stream for the full N-epic run.
3. `soft-scope-budget-policy` and `review-convergence-adjudicator` reduce false stops and expensive reviewer churn without weakening hard safety gates.
4. Shared runtime is next because the `governor-state-refresh` cycle still needed manual recovery when isolated worktree verification could not find `.venv`.
5. Environment repair, liveness, artifact ownership, and onboarding then harden operations for repeated unattended runs.
