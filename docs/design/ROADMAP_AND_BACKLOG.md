...
- The five-cycle dogfood review is captured in `FIVE_CYCLE_DOGFOOD_REVIEW.md`.
  It measured 7 release attempts, 51 worker/reviewer task runs, 183.5 minutes
  of executor time, 28.0 minutes of verification time, 1.84M prompt characters,
  1.57M injected context characters, and 25.3MB of raw stderr/audit chatter.
  The measured bottlenecks are shared verification runtime drift, repeated
  review convergence churn, coarse context slicing, incomplete cost governance,
  and manual finalization/state-update work.
...
- Project configs support `model_roles`, `model_routing`, and `model_catalog` for capability-aware routing. Default configs express the recommended hierarchy of `gpt-5.5` strategic planning, `gpt-5.2` runtime/review supervision, `gpt-5.3-codex-spark` coding work and micro repair, plus `gpt-5.3-codex` as the sturdier coding fallback and `gpt-5.4-mini` cheap routing/fallback. `doctor` reports unsupported, unknown, and uncataloged role models so routing can degrade safely.
...
- Generated-contract admission rejects hard safety violations such as release mismatch, missing diff evidence, weak stop conditions, whole-repo scope, unknown verification profiles, and inconsistent verification profiles. Allowed-file counts and diff-size pressure are implemented scope-risk findings; severe violations remain hard stops, while modest overages should be reviewed by the governor/supervisor agent with an auditable accept, split, rerun, or escalation decision after the hard gates pass.
...
- `agent-loop run-governor --epic-count N` is the shipped repeated-cycle shell. It composes repeated one-epic cycles with parent governor log/events artifacts, repo-state completed-epic updates, recent release-summary references, and default stops for planning-only or non-accepted cycles.
...
- `run-backlog` writes governor-level log artifacts for a one-epic invocation. `run-governor` writes a parent governor log/events stream for repeated cycles, and the post-cycle refresh path now writes `state_refresh_summary.json` and `post_cycle_state_refresh.json` on success or `state_refresh_error.json` on failure so durable state can be compacted before the next selection step.
- Release planning now has deterministic state-review snapshot capture (`state_review_snapshot.json`) plus contract-plan schema support for `state_review_snapshot_path`; full agent-driven pre-epic state-review decisioning over live repository evidence is still pending.
...
- The soft-gates dogfood run showed the planner-output normalization gap that led to this release: the generated contracts were semantically usable but missed required `git diff`/changed-files evidence and used worktree-local `.venv` verification commands. The shipped typed planner normalization seam should repair those bounded defects, rerun deterministic contract admission, and continue without manual materialization; reviewer-output and supervisor-output normalization remain planned extensions on the same seam.
...
- The `review-loop-convergence-policy` dogfood run closed one release-local brittleness point by hardening the final review stop policy around final integration verification, explicit blocker versus deferred classification, and repo-state compaction. The broader N-epic governor still needs to consume those decisions across repeated epics, but the release-local stop boundary itself is implemented.
...
- The `multi-epic-run-governor-hardening` dogfood cycle showed that the system can now autonomously execute a sizable epic, serialize overlap-heavy work, run a release-local semantic review, generate bounded repair contracts, and merge accepted repair work into the feature branch. It also showed the next brittleness layer: planner-output normalization is still reactive to new schema drift, repeated reviewer waves can become expensive, final review can escalate on missing integration-branch verification evidence or truncated review context even when the branch is locally green, and state-refresh failures need first-class error artifacts instead of silent or partial state. The current consequence is architectural, not just tactical: the governor needs an integration-review/evidence service that runs final verification on the feature branch, packages the full diff/context for the reviewer, records review limitations as typed findings, and decides whether remaining findings are blockers, accepted risks, backlog follow-ups, or manual-merge completion outcomes that should compact into repo-state before the next epic.
- The `planner-normalization-generalization` dogfood cycle shipped the reusable typed planner-output normalization path, but exposed five higher-level autonomy gaps. First, tracked repo-state can become stale after manual merges and cause the next governor cycle to reselect completed work. Second, self-development dirties the target checkout with generated objectives/contracts/repo-state before execution; this is acceptable only if the governor owns planning-package commits, while external targets should keep durable artifacts in target-owned `.auto_develop/` state or a control repo. Third, the operator still has to correlate parent governor logs, child release logs, raw logs, and review artifacts; the product target is one tail-able `governor.log` cockpit for the complete N-epic run. Fourth, hard file-count budgets were too crude and are now handled by the soft scope-risk policy as supervisor-adjudicated findings after hard gates pass, not automatic hard stops. Fifth, repeated reviewer waves can discover adjacent issues indefinitely; after bounded repair attempts, the supervisor should run final verification and accept, defer, or block remaining findings with typed rationale.
- The five-cycle dogfood effort confirmed that the system can implement sizable
  epics with accepted task work, review repair, and final verification, but not
  yet as an unattended long-running loop. Manual work was still needed to repair
  worktree runtime assumptions, commit planning packages, accept soft scope risk,
  stop review churn, merge/delete branches, and compact repo-state memory.

Prioritized Phase 4 epics:

Completed: `governor-state-review` added deterministic pre-epic state-review snapshot capture and backlog-planning evidence linkage. `agentic-feature-review-loop` added the release-local semantic review gate with structured reviewer artifacts, bounded repair contracts, verification reruns, and finalization gating. `persistent-governor-memory` added durable backlog/repo-state memory seams. `supervisor-decision-records` added typed, auditable supervisor decision records and the first soft-budget runtime consumer. `supervisor-owned-release-scheduling` is implemented for normal source overlap, with follow-up schema/review hardening still useful before broader rollout. `supervisor-execution-strategy` is implemented in the one-epic planning path as the shipped strategy seam. `model-output-normalization` is implemented for bounded planner, reviewer, and supervisor artifact normalization with strict validator reruns and refusal boundaries. `review-loop-convergence-policy` is implemented for release-local duplicate, soft-finding, false-positive, scope-expansion, and backlog-follow-up classification. `multi-epic-run-governor-hardening` added per-cycle state-refresh summaries, typed governor stop reasons, no-actionable-work detection, accepted-release finalization/cleanup handoff metadata, feature-review continuation metadata, parent child-artifact manifests, and state-refresh failure evidence. `autonomous-finalization-and-cleanup` added policy-owned finalization decisions, PR handoff artifacts, cleanup eligibility from finalization evidence, state-memory updates, and safer branch/worktree cleanup. `planner-normalization-generalization` added the reusable typed planner-output normalization decision path and strict validator rerun evidence. `governor-state-refresh` added durable state-refresh models, post-cycle refresh payloads, refresh failure artifacts, continuation metadata, and documented accepted/failed/blocked/manual-merge state compaction behavior. `planner-admission-repair` added structured supervisor inputs for contract admission failures, typed admission-repair decisions, bounded repair/rerun behavior, persisted release artifacts, hard safety tests, docs, and reviewer-driven repair fixes. `governor-cockpit-v2` added parent governor cockpit events, typed event context, linked child artifact manifests, stop-context rendering, docs, and smoke/full-suite coverage. `soft-scope-budget-policy` added typed scope-risk budget policy decisions, release-confined evidence copies, structured metric derivation, deterministic blocking placeholders, and tests so file/diff budgets are no longer universal hard stops. `review-convergence-adjudicator` added typed final review finding adjudication artifacts, final integration verification ordering, compact final-review follow-up memory, and tests; its dogfood cycle also proved that repeated reviewer waves still need a stronger governor-owned review-stop policy to prevent expensive churn. Historical completed objectives, contracts, and raw runs are no longer retained as active planning inputs in the controller repo.

1. `shared-verification-runtime`: add shared verification-runtime configuration so isolated worktrees can run tests through a known Python/toolchain without requiring a per-worktree `.venv`.
...
4. Implement integration-review evidence handoff and autonomous finalization before claiming multi-epic unattended operation; otherwise the governor can implement epics but still needs a human to decide whether and how to land them.
5. Add state refresh, planner admission repair, and review adjudication before expecting unattended multi-epic operation; otherwise the governor still needs human intervention after common bounded failures.
6. Add shared verification runtime and environment repair close behind, because repeated worktree test failures are a known source of unnecessary stops.
7. Expand the cockpit log as soon as multiple child artifacts exist, otherwise operators cannot monitor long runs coherently.
