...
7. `governor-state-refresh`: teach the governor to apply or commit policy-compliant roadmap/backlog/repo-state updates after each epic with outcome, metrics, lessons, review findings, and next recommendations.
...
1. `planner-normalization-generalization` is now the central product gap. Repeated dogfood cycles keep revealing repairable planner/schema drift; one-off parser patches are useful but should shrink behind a reusable supervisor normalization interface. Reviewer-output and supervisor-output normalization remain planned until the same typed seam is extended to those artifacts.
2. `governor-cockpit-v2`, shared runtime, environment repair, liveness, artifact ownership, state refresh, and onboarding then harden operations for repeated unattended runs.
