...
8. `onboarding-bootstrap`: add a bootstrap/onboarding command or checklist that turns a freshly cloned target repo plus one or two prompts into the required config, repo-state memory, objective/backlog directories, and initial doctor checks.

Current priority rationale:

1. `governor-cockpit-v2` is now first because manual monitoring still requires correlating parent governor logs, child release logs, feature review artifacts, and state updates. The target operator experience is one colorful, human-facing cockpit stream for the full N-epic run.
2. `soft-scope-budget-policy` and `review-convergence-adjudicator` reduce false stops and expensive reviewer churn without weakening hard safety gates.
3. Shared runtime remains urgent because the `planner-admission-repair` cycle still needed manual contract patching to avoid worktree-local `.venv` failures.
4. Environment repair, liveness, artifact ownership, and onboarding then harden operations for repeated unattended runs.
