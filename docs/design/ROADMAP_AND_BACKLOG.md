...
1. `governor-state-refresh` is now the first operational gap because stale tracked state can cause wrong next-epic selection. The top-level loop must own state compaction after both autonomous finalization and manual merges.
2. `planner-admission-repair` follows because the planner can still emit semantically useful but admission-invalid contract packages. A high-level supervisor should repair/split/narrow/replan instead of requiring manual materialization.
3. `governor-cockpit-v2` follows because multiple child logs are already operationally expensive. The target operator experience is one colorful, human-facing cockpit stream for the full N-epic run.
4. `soft-scope-budget-policy` and `review-convergence-adjudicator` reduce false stops and expensive reviewer churn without weakening hard safety gates.
5. Shared runtime, environment repair, liveness, artifact ownership, and onboarding then harden operations for repeated unattended runs.
