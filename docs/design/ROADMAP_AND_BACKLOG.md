...
1. `planner-normalization-generalization` is now the central product gap. Repeated dogfood cycles keep revealing repairable planner/reviewer/schema drift; one-off parser patches are useful but should shrink behind a reusable supervisor normalization interface.
