...
1. `multi-epic-run-governor-hardening` is now the central product gap. State review, persistent memory, supervisor decisions, release scheduling, execution-strategy selection, model-output normalization, review-loop convergence, and the repeated-cycle shell are in place; the remaining work is hardening state refresh, finalization, cleanup, and stop criteria for unattended multi-epic operation.
