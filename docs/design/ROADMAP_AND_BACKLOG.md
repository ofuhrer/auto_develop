...
1. `integration-review-evidence-handoff` is now the central product gap exposed by the latest full cycle. The implementation work and bounded repairs can complete autonomously, but final acceptance still depends on packaging enough context and verification evidence for the reviewer/supervisor to make a durable decision.
2. `autonomous-finalization-and-cleanup` is the next land-the-work gap. Without it, a green and accepted feature branch still needs a human to merge, push, delete branches, and update durable memory.
3. `planner-normalization-generalization` should follow because repeated dogfood cycles keep revealing new repairable planner/schema drift. One-off parser patches are useful but should shrink behind a supervisor normalization interface.
4. `governor-cockpit-v2`, shared runtime, environment repair, liveness, artifact ownership, state refresh, and onboarding then harden operations for repeated unattended runs.
