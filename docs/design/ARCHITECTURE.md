...
Build an autonomous-first external control layer around existing coding agents. The deterministic kernel owns hard safety invariants, Git/worktree mechanics, state transitions, verification, evidence, metrics, typed artifact persistence, and explicitly configured finalization. A high-level governor/supervisor agent owns judgment-heavy decisions: choosing the next epic, choosing whether to use one-shot implementation or decomposed sub-agents, deciding the execution DAG, normalizing useful-but-invalid planner output into strict typed artifacts, adjudicating reviewer findings, selecting repair/retry/split/accept strategies, and updating roadmap memory. Coding agents own implementation inside the strategy selected by the supervisor. Humans define goals, credentials, and hard safety constraints; they should not be routine approval gates inside the development loop. The one-epic strategy seam is already shipped: the planner path can select one-shot, sequential, parallel, stacked, patch handoff, replan, or stop before contract generation. It persists selected executable/replan actions as typed supervisor evidence and always writes `execution_strategy_selection.json`; `stop` currently has selection JSON only. Today `one_shot` materializes a bounded execution-input artifact and stops before contract generation; the worker execution path for that artifact remains planned. The repeated-cycle `run-governor --epic-count N` shell is also shipped: it composes repeated one-epic cycles, records parent governor logs/events, marks completed epics, and stores recent release summaries. The broader N-epic orchestration layer that would add full pre-epic live state review, final-review continuation, and policy-driven cleanup remains planned.

The implemented planner normalization path is a typed decision seam. It records raw planner artifact references, validation errors, the selected action, fallback plan, validators to rerun, and normalized artifact references. The shipped admission-repair flow uses that seam for repairable generated-contract failures: add missing evidence, normalize worktree-local verification commands, preserve scope and intent, and rerun deterministic contract validation before execution continues. It refuses repairs that would broaden scope, change intent, or invent missing hard evidence, and it hard-stops on unsafe release IDs, forbidden path changes, generated-artifact or lockfile or migration changes, missing hard evidence, weak stop conditions, or whole-repo scope. Reviewer-output and supervisor-output normalization remain planned extensions on this same seam unless a release explicitly says otherwise.

The architecture should optimize for autonomous development outcomes, not for maximizing the number of agents. A capable one-shot implementation agent is a first-class execution strategy for cohesive medium-sized architectural work. Decomposition into contracts and parallel workers is useful when work is genuinely independent, risky, or needs isolation, but it should be selected by the supervisor rather than imposed by default.
...
The default posture is autonomous-first and agentic-first. Once a repository goal and safety policy are configured, the governor should first perform a repository state-review pass: inspect docs, roadmap state, repo-state memory, recent run summaries, release artifacts, metrics, tuning reports, current branch state, relevant source layout, and open work. It should then select the next highest-reward epic and choose an execution strategy: one-shot high-capability implementation for cohesive work, sequential contracts for dependent slices, parallel sub-agents for independent low-overlap work, stacked branches for true dependencies, patch handoff for risky exploration, or replanning when the scope is wrong. The runtime supervisor should observe release events and evidence, classify recoverable failures, normalize malformed but useful planner output into typed artifacts with validator-rerun metadata, apply bounded repair actions, and resume execution. The implemented control plane is narrower than that target: it now has a one-epic `GovernorLoop` service boundary, a typed `StateStore` seam, a `RepairPolicy` seam, planner-output normalization, governor-level logging for `run-backlog`, runtime-supervisor repair/resume seams for structured release failures, deterministic state-review snapshot capture (`state_review_snapshot.json`) with contract-plan linkage (`state_review_snapshot_path`), persistent governor memory seams, typed supervisor decision records, supervisor-owned release scheduling for normal source overlap, supervisor-owned execution-strategy selection, one-shot execution input materialization, phase-aware context bundles with manifest evidence and size/truncation records, post-cycle state-refresh artifacts, and a release-local feature-review/repair loop when `model_roles.reviewer` is configured. For executable `run-backlog` cycles, the governor currently forces contract decomposition until a one-shot worker runner exists; explicit one-shot selection is planning/input materialization and returns no `ReleaseRunResult`. That review loop is the shipped semantic gate for one integrated feature branch, but it is not yet composed into a persistent N-epic orchestration layer and its raw reviewer-output schema remains too brittle. Required findings become bounded repair contracts, the reviewer re-check reruns verification, and finalization is blocked until required findings are resolved or explicitly accepted with rationale. Normalization may repair schema shape, backfill derivable evidence, or rewrite worktree-local verification commands to the configured shared runtime, but if doing so would broaden scope, change intent, or invent missing hard evidence, the supervisor must stop. The bundle machinery is intentionally narrow: it shapes context for a given phase, but it is not semantic retrieval/search, embeddings, or broad cross-run memory. After each cycle, the governor writes state-refresh evidence and applies compact durable state updates before selecting the next epic; if refresh fails, it writes first-class error evidence and stops before the next selection step. Accepted/finalized, failed/blocked, and manual-merge outcomes are carried through the refresh trail, but full pre-epic state-review decisioning, broader N-epic loop, one-shot worker execution, typed blocked-decision persistence for `stop`, always-on state refresh, and broader retrieval features are still planned extensions on top of those seams.
...
The deterministic kernel must remain strict about invariants: no skipped verification, no silent policy expansion, no broad file-scope changes, no unapproved destructive operations, and no acceptance based only on worker claims. The runtime supervisor may repair contracts, environment setup, task splits, and retry plans, but repaired artifacts must pass deterministic admission and verification before execution continues. Generated-contract admission repair is bounded, evidence-backed, and validator-driven; it does not bypass the hard gates.

Verification-environment repair is the narrow implemented environment-setup seam inside that boundary: it can declare a policy-bounded `apply_repair_and_retry` or `capture_evidence_only` action, persist `verification_environment_repair_input.json` and `environment_repair__*.json`, consume retry budget when it retries, and rerun the task's verification validators. It is not executor-liveness supervision or broad infrastructure repair, and it does not install dependencies, repair credentials, repair network access, or mutate source files.

Planner-output repair is part of the autonomous path. If a planner emits a useful but admission-invalid contract package, the system should not stop immediately. The supervisor should receive the raw planner output, validation errors, objective context, config, schema, and repository policy, then apply bounded normalization that does not change meaning: add required evidence such as `git diff` and changed-files lists, normalize worktree-local `.venv` verification commands to the configured shared runtime, repair schema spelling/shape drift, fill derivable evidence paths from the objective context, and preserve objective, allowed scope, forbidden changes, stop conditions, and evidence intent. The repaired artifact must then pass deterministic validation, and the hard gates must be rerun against that repaired artifact before execution continues. If normalization would broaden scope or change intent, or would need to invent missing evidence, the supervisor must stop with evidence. Reviewer-output and supervisor-output normalization extensions should stay documented as planned until code and tests explicitly ship them.
