...
- The latest `multi-epic-run-governor-hardening` cycle proved that autonomous implementation, task-level verification, semantic review, and bounded repair waves can operate end-to-end on a sizable feature branch. It also exposed a remaining architectural seam: final integration acceptance is not yet a first-class service. The reviewer escalated on truncated diff context and missing integration-branch verification evidence even though local verification later passed. That means the system can do the work but does not yet reliably package the whole feature branch, full diff context, final verification logs, reviewer limitations, and accepted-risk decisions into one durable integration-review handoff.
- Planner-output normalization is still implemented as a growing set of local parser repairs. The cycle needed additional normalization for implementation-requirements fields, wrapper-level dependencies, tail-brace JSON defects, and descriptive task-type aliases. This confirms that normalization should be a typed supervisor repair action with validator reruns, not a sequence of ad hoc parser exceptions.
- Bounded repair waves are useful but expensive. The cycle fixed real issues over two review waves, then escalated on evidence/context concerns. Review convergence therefore needs an explicit economic and semantic boundary: new findings after successful repairs must be classified as blocker, accepted risk, duplicate, scope expansion, or backlog follow-up using full integration evidence.
...
- Add an integration-review evidence service between release execution and finalization. It should run configured final verification on the feature branch, collect complete branch diff/context, package reviewer inputs, persist review limitations, and convert the reviewer recommendation into a typed governor continuation decision.
- Replace one-off planner parser repairs with a supervisor-owned normalization interface for bounded raw-output defects. Deterministic strict schemas should remain the final gate, but raw model output should first pass through a typed repair/refusal decision.
