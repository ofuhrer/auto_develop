...
`run-governor` now creates a parent `governor.log`/`events.jsonl` stream for repeated epic cycles. It reuses the current one-epic planner/objective/release machinery, marks completed epics in repo state, records recent release summaries, and stops on planning-only or non-accepted cycles unless policy allows continuation. The shipped feature-review pass runs only when `model_roles.reviewer` is configured and remains release-local; autonomous final-review repair continuation, final branch cleanup, and full pre-epic state-review decisioning remain planned.
